...
module.exports = function register(api) {
  // Tool 1: list_directory — List directory contents with details
  api.registerTool("list_directory", {
    name: "list_directory",
    description:
  ...
  // Tool 2: create_file_or_dir — Create files or directories
  api.registerTool("create_file_or_dir", {
  ...
  // Tool 3: copy_file — Copy a file or directory
  api.registerTool("copy_file", {
    name: "copy_file",
  ...
  // Tool 4: move_file — Move or rename a file/directory
  api.registerTool("move_file", {
    name: "move_file",
  ...
  // Tool 5: delete_file — Delete a file or directory (with safety check)
  api.registerTool("delete_file", {
    name: "delete_file",
    description:
    ...
    async execute(_id, params) {
      ...
      if (forbidden.includes(target)) {
        return {
          content: [{ type: "text", text: `Error: Refusing to delete system directory: ${target}` }],
        };
      }
  ...
  // Tool 6: find_files — Find files by name, extension, size, or modification time
  api.registerTool("find_files", {
    name: "find_files",
    description:
      "Find files matching criteria: name pattern, extension, size, or modification time.",
    inputSchema: {
      type: "object",
      properties: {
        dirPath: {
    ...
    async execute(_id, params) {
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: result.trim() || "No files found." }],
        };
      } catch (err) {
        return {
  ...
  // Tool 7: disk_usage — Check disk usage and file sizes
  api.registerTool("disk_usage", {
    name: "disk_usage",
