...
module.exports = function register(api) {
  // Tool 1: read_file — Read full or partial file contents
  api.registerTool("read_file", {
    name: "read_file",
    description:
      "Read the contents of a file. Supports text, code, data, config, and markup files. " +
      "Can read the full file or a specific line range for large files.",
    inputSchema: {
      type: "object",
      properties: {
        filePath: {
    ...
    async execute(_id, params) {
      ...
      if (stats.isDirectory()) {
        return {
          content: [{ type: "text", text: `Error: ${resolved} is a directory, not a file.` }],
        };
      }
  ...
  // Tool 2: preview_file — Quick file preview with metadata
  api.registerTool("preview_file", {
    name: "preview_file",
    description:
  ...
  // Tool 3: search_in_file — Search for patterns within files
  api.registerTool("search_in_file", {
    name: "search_in_file",
    description:
    ...
    async execute(_id, params) {
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: `Error running search: ${err.message}` }],
        };
      }
    },
  });

  // Tool 4: compare_files — Diff two files
  api.registerTool("compare_files", {
    name: "compare_files",
    ...
    async execute(_id, params) {
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: result || "Files are identical." }],
        };
      } catch (err) {
