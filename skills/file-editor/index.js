...
module.exports = function register(api) {
  // Tool 1: replace_text — Find and replace text in a file
  api.registerTool("replace_text", {
    name: "replace_text",
    description:
      "Find and replace text in a file. Supports single or global replacement, " +
      "with optional regex mode. Shows a preview of changes before applying.",
    inputSchema: {
      type: "object",
      properties: {
        filePath: {
    ...
    async execute(_id, params) {
      ...
      // Count replacements
      const matches = original.match(new RegExp(regex.source, regex.flags + (regex.flags.includes("g") ? "" : "g")));
  ...
  // Tool 2: insert_lines — Insert content at a specific position
  api.registerTool("insert_lines", {
    name: "insert_lines",
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lineNumber: {
          type: "number",
          description: "Line number (1-based) for before_line/after_line positions",
        },
        pattern: {
          type: "string",
          description: "Pattern to match for after_pattern position",
        },
      },
  ...
  // Tool 3: delete_lines — Delete lines from a file
  api.registerTool("delete_lines", {
    name: "delete_lines",
    description:
      "Delete specific lines from a file by line number, range, or matching pattern. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        endLine: {
          type: "number",
          description: "End line number (1-based, same as startLine to delete a single line)",
        },
        pattern: {
          type: "string",
          description: "Delete all lines matching this pattern (alternative to line numbers)",
        },
        dryRun: {
          type: "boolean",
          description: "Preview deletions without applying (default: false)",
        },
      },
      required: ["filePath"],
    },
    async execute(_id, params) {
      ...
      if (pattern) {
        toDelete = lines
          .map((l, i) => (l.includes(pattern) ? i : -1))
          .filter((i) => i !== -1);
        ...
        return {
          content: [{ type: "text", text: "Error: Provide either startLine+endLine or pattern." }],
        };
      }
  ...
  // Tool 4: transform_file — Transform file content (encoding, line endings, case, whitespace)
  api.registerTool("transform_file", {
    name: "transform_file",
    description:
      "Transform file content: convert line endings, change case, remove trailing whitespace, or change encoding.",
    inputSchema: {
      type: "object",
      properties: {
        filePath: {
    ...
    async execute(_id, params) {
      ...
      return {
        content: [{ type: "text", text: `${description} in ${resolved}.` }],
      };
    },
  });
};
